    ...
    async def _handle_wechat_login_cmd(raw_args: str) -> str:
        ...
        try:
            async with aio.ClientSession(trust_env=True, connector=ssl_ctx) as session:
                ...
                # Generate QR code image as base64 data URL for WebUI
                qr_data_url = None
                try:
                ...
                # Build response: text link + ASCII QR for terminal
                result = (
                    f"📱 请用微信扫描以下二维码登录：\n\n"
                    f"{qr_link}\n\n"
                )
                # Add ASCII QR code for CLI/terminal display
                try:
                    import qrcode as _qrcode
                    ...
                    buf = StringIO()
                    qr_img.print_ascii(out=buf, invert=True)
                    ascii_qr = buf.getvalue()
                    result += f"```\n{ascii_qr}\n```\n\n"
                except Exception:
                    pass  # Skip ASCII QR if generation fails
                result += (
                    f"⏳ 二维码5分钟内有效，请尽快扫描。\n\n"
